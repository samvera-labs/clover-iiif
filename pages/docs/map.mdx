...
Zooming with the mouse wheel or trackpad is **off by default**. A map placed partway down
a scrolling page would otherwise swallow the wheel and trap the reader — the same reason
Clover disables OpenSeadragon's `scrollToZoom` in `Image` and `Viewer`. The zoom controls,
double-click and pinch gestures all still work.

Turn it on with `scrollZoom` for a full-page or full-bleed map, where wheel zoom is what a
reader expects:

```jsx
<Map iiifContent={iiifContent} fitToData scrollZoom />
```

<Callout type="info">
  Prior to this release the map zoomed on scroll, following MapLibre's own
  default. If you are relying on that behavior, add `scrollZoom` to keep it.
</Callout>
