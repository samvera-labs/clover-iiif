...
Render the slider with IIIF Collection URI. The only required prop is the `collectionId` prop, which is the URI of the IIIF Collection.
...
const { Slider } = dynamic(() => import("@samvera/clover-iiif"), {
  ssr: false,
});

return <Slider collectionId={collectionId} />;
```
...
```tsx
const MyCustomSlider = ({ collectionId }) => {
