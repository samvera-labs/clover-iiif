...
Slides are sized by their own content, and the track is the sum of their widths — so how many are visible follows from the viewport rather than being divided into it. There is deliberately no `slidesPerView`. Set the card width in CSS:

```css
.my-app {
  --clover-slider-item-width: 12rem; /* defaults to 15rem */
}
```

Cards are square, sized from that width. `--clover-slider-item-width` is the Slider's own knob; without it the card falls back to the library-wide `--clover-thumbnail-width`, so one variable can scale every thumbnail at once. See [Thumbnail size](/docs/viewer#thumbnail-size).
