...
| `customTheme`                    | Partial theme token overrides (see [Custom Theme](#custom-theme))                                                                               | No       |                                          |
...
Override any of Clover's design tokens by passing a `customTheme` object. Each key maps to a CSS custom property (`colors.foreground` → `--colors-foreground`), so anything you provide becomes available to every component. Legacy `primary`/`secondary` keys still work and are automatically mapped to the new `foreground`/`background` tokens.
...
const customTheme = {
  colors: {
    /**
     * Foreground (text, icons) should contrast with every background token.
     */
    foreground: "#37474F",
    foregroundMuted: "#546E7A",
    foregroundAlt: "#263238",

    /**
     * Accent is reserved for actionable elements (buttons, toggles, focus states).
     */
    accent: "#C62828",
    accentMuted: "#E57373",
    accentAlt: "#B71C1C",
    ...
    /**
     * Background surfaces live behind content. Each one must meet WCAG contrast against `foreground` and `accent`.
     */
    background: "#FFFFFF",
    backgroundMuted: "#ECEFF1",
    backgroundAlt: "#CFD8DC",
  },
};
