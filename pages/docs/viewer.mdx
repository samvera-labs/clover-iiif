...
People can use prebuilt plugins or create their own plugins. To install a prebuilt plugin, people should follow the instructions for the particular plugin. If you install multiple plugins, make sure the plugin ids are unique.
...
Here is an example plugin that has both `imageViewer.controls` and `informationPanel`
