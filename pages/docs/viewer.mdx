...
The information panel is a collapsible panel that displays information about the current Manifest and renders annotations and IIIF Content Search for the active canvas. It is rendered by default, but can be configured to hide About or opt out of individual data-backed tabs.
...
Panel visibility is based on the content Clover can actually render:

- About makes the panel visible when `renderAbout` and the current open state are both true.
- Available annotations make the panel visible when `renderAnnotation` is true, even if `open` or `renderAbout` is false.
- An available IIIF Content Search service makes the panel visible when `renderContentSearch` is true, even if `open` or `renderAbout` is false.

`open` supplies the initial open state; it is not an override that hides available Annotations or Content Search. Likewise, `renderToggle` controls whether Clover renders its open/close control, not whether the panel has content. To suppress the panel for a Manifest that provides annotations or Content Search, explicitly set the corresponding `renderAnnotation` or `renderContentSearch` option to `false`.
