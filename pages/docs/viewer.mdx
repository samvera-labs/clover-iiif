...
If your site has a search functionality that returns a list documents that match a search term, and you want to highlight that search term when the Clover viewer loads, use `iiifContentSearchQuery` when setting up the viewer.
...
You can set the appearance of how the search results are highlighted using `options.contentSearch.overlays`. By default the search results have a light red highlight.

If you don't want the search results to be highlighted, set `options.contentSearch.overlays.renderOverlays` to `false`.
