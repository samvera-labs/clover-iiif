...
export const Navigator: React.FC<NavigatorProps> = ({ currentTime }) => {
  return (
    <NavigatorWrapper>
      ...
      <NavigatorBody>
        <NavigatorOutput>
          <NavigatorBodyInner items={sampleParsedVtt.output} currentTime={currentTime} />
        </NavigatorOutput>
      </NavigatorBody>
    </NavigatorWrapper>
  );
...
const NavigatorBodyInner: React.FC<NavigatorBodyInnerProps> = ({ items, currentTime }) => {
  return (
    <>
      {items.map(({ id, text, startTime }) => (
        <NavigatorItem label={text} startTime={startTime} t={currentTime} key={id} />
      ))}
    </>
  );
};
